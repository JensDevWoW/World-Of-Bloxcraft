...
function Player.new(link, char, target, unitLink)
	...
	new_player.SpellList = new_player.bp["Spell List"]
...
function Player:IsInDuel(target)
	if self:GetDuel() then
		if self.m_duelObject.target.link == target.link or self.m_duelObject.initiator == target.link then
...
function Player:GetSpellFromKey(key)
	local spellI = self.SpellList:findFirstChild(tostring(key))
	local spellId = spellI.Value
	if spellId ~= nil then
		for _,v in next,game:service'Lighting'.Spells:GetChildren() do
